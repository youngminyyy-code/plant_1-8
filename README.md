...
학생은 글을 쓸 때 둘 중 하나를 선택해서 이야기를 남기고, 응원도 식물별로 따로 쌓여요.

- **🌱 오늘의 이야기** (`/`): 식물 탭(바줘/로베르토) + 날짜(◀ ▶/달력)로 그날 그 식물에
  달린 글만 모아 보여줘요.
- **🌻 성장 현황** (`/growth`): 두 식물이 각자의 카드로 나란히 나와요. 식물 몸(0~5단계, 잎
  개수·꽃/열매)은 관리자가 올려주고, 화분 얼굴 표정은 **그 식물에 달린 전체 댓글 수**에 따라
  자동으로 바뀌어요 (0개 무표정 → 1~3개 미소 → 4~10개 밝은 미소 → 11개 이상 활짝 웃음+반짝임).
  반응(응원+댓글)이 많은 이야기일수록 캐릭터 주변에 더 크고 반짝이게 떠 있어요.
...
4. 다시 SQL Editor로 돌아와서 [`supabase/schema.sql`](supabase/schema.sql) 파일 내용을 전체 복사해 붙여넣고 **Run** 클릭
   ...
   - 마지막으로 [`supabase/migration_3_plants.sql`](supabase/migration_3_plants.sql) 도 **새 쿼리**로 실행해주세요
     (식물 2개 구분에 필요해요)
